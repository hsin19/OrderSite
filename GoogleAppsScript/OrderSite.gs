class OrderSide {
    ...
    _createSellListPage() {
        ...
        sh.getRange(1, 1).setValue(
            `=QUERY(Products!A:G,"SELECT A,B,E,F WHERE A IS NOT NULL AND B>0 AND E>0 AND G='Publish' LABEL E 'Quantity'",1)`
        );
        sh.hideSheet();
        sh.protect().setWarningOnly(true);
    }
    ...
    _createSystemPage() {
        ...
        sh = this._ss.insertSheet(shName);
        let data = [
            `=QUERY({arrayformula(row(Products!G:G)), Products!G:G},"SELECT Col1 WHERE Col2='Delete'",0)`,
    ...
    endOrder(row, reduceStock = true) {
        ...
        shTo.getRange(rowFrom, length + 1, rowNum)
            .merge()
            .setValue(dateTime);
    ...
    fillProduct() {
        ...
        if (
            template.getFormulas()[0][0] !== `=SUMIF(Orders!B:B,"="&OFFSET($A$1,ROW()-1,0),Orders!C:C)` ||
            template.getFormulas()[0][1] !== `=OFFSET($C$1,ROW()-1,0)-OFFSET($D$1,ROW()-1,0)`
        ) {
            template.setValues([
                [
                    `=SUMIF(Orders!B:B,"="&OFFSET($A$1,ROW()-1,0),Orders!C:C)`,
                    `=OFFSET($C$1,ROW()-1,0)-OFFSET($D$1,ROW()-1,0)`,
                ],
            ]);
        }
